...
(ns ^{:author "Gary Fredericks"
      :doc "Internal namespace, wrapping some Long functionality for nbb.
            Uses the npm 'long' package instead of goog.math.Long."}
...
(defn *
  [x y]
  (let [a48 (core/unsigned-bit-shift-right (.-high x) 16)
        a32 (bit-and (.-high x) 0xFFFF)
        a16 (core/unsigned-bit-shift-right (.-low x) 16)
        a00 (bit-and (.-low x) 0xFFFF)

        b48 (core/unsigned-bit-shift-right (.-high y) 16)
        b32 (bit-and (.-high y) 0xFFFF)
        b16 (core/unsigned-bit-shift-right (.-low y) 16)
        b00 (bit-and (.-low y) 0xFFFF)

        arr (array 0 0 0 0)]                              ;[c00 c16 c32 c48]
    (aset arr 0 (core/* a00 b00))                                                  ;c00 += a00 * b00;
    (aset arr 1 (core/unsigned-bit-shift-right (aget arr 0) 16))                       ;c16 += c00 >>> 16
    (aset arr 0 (bit-and (aget arr 0) 0xFFFF))                                     ;c00 &= 0xFFFF;
    (aset arr 1 (core/+ (aget arr 1) (core/* a16 b00)))                            ;c16 += a16 * b00;
    (aset arr 2 (core/unsigned-bit-shift-right (aget arr 1) 16))                       ;c32 += c16 >>> 16;
    (aset arr 1 (bit-and (aget arr 1) 0xFFFF))                                     ;c16 &= 0xFFFF;
    (aset arr 1 (core/+ (aget arr 1) (core/* a00 b16)))                            ;c16 += a00 * b16;
    (aset arr 2 (core/+ (aget arr 2) (core/unsigned-bit-shift-right (aget arr 1) 16))) ;c32 += c16 >>> 16;
    (aset arr 1 (bit-and (aget arr 1) 0xFFFF))                                     ;c16 &= 0xFFFF;
    (aset arr 2 (core/+ (aget arr 2) (core/* a32 b00)))                            ;c32 += a32 * b00;
    (aset arr 3 (core/unsigned-bit-shift-right (aget arr 2) 16))                       ;c48 += c32 >>> 16;
    (aset arr 2 (bit-and (aget arr 2) 0xFFFF))                                     ;c32 &= 0xFFFF;
    (aset arr 2 (core/+ (aget arr 2) (core/* a16 b16)))                            ;c32 += a16 * b16;
    (aset arr 3 (core/+ (aget arr 3) (core/unsigned-bit-shift-right (aget arr 2) 16))) ;c48 += c32 >>> 16;
    (aset arr 2 (bit-and (aget arr 2) 0xFFFF))                                     ;c32 &= 0xFFFF;
